    ...
    if self.state == "MAIN" then
        if Input.pressed("confirm") then
            ...
            elseif self.currently_selected == 5 then
                WidescreenLib.widescreen = not WidescreenLib.widescreen
                if not WidescreenLib.widescreen then
                    WidescreenLib:toInitScreen()
                else WidescreenLib:toWideScreen() end
...
function DarkConfigMenu:draw()
    ...
    if self.state ~= "CONTROLS" then
        ...
        love.graphics.print("Master Volume",   88, self.choff + (0 * self.chdist))
        Draw.setColor(PALETTE["world_text"])
        love.graphics.print("Controls",        88, self.choff + (1 * self.chdist))
        love.graphics.print("Simplify VFX",    88, self.choff + (2 * self.chdist))
        love.graphics.print("Fullscreen",      88, self.choff + (3 * self.chdist))
        love.graphics.print("Widescreen",      88, self.choff + (4 * self.chdist))
        love.graphics.print("Auto-Run",        88, self.choff + (5 * self.chdist))
        love.graphics.print("Return to Title", 88, self.choff + (6 * self.chdist))
        love.graphics.print("Back",            88, self.choff + (7 * self.chdist))

        if self.state == "VOLUME" then
            Draw.setColor(PALETTE["world_text_selected"])
        end
        love.graphics.print(Utils.round(Kristal.getVolume() * 100) .. "%",      348, self.choff + (0 * self.chdist))
        Draw.setColor(PALETTE["world_text"])
        love.graphics.print(Kristal.Config["simplifyVFX"] and "ON" or "OFF", 348, self.choff + (2 * self.chdist))
        love.graphics.print(Kristal.Config["fullscreen"]  and "ON" or "OFF", 348, self.choff + (3 * self.chdist))
        love.graphics.print(WidescreenLib.widescreen  and "ON" or "OFF", 348, self.choff + (4 * self.chdist))
        love.graphics.print(Kristal.Config["autoRun"]     and "ON" or "OFF", 348, self.choff + (5 * self.chdist))

        Draw.setColor(Game:getSoulColor())
        Draw.draw(self.heart_sprite,  63, self.choff + 10 + ((self.currently_selected - 1) * self.chdist))
    else
